...
export const Gradient = defineClass({
  ...
  */
  ...
  drawGradient: function (element, gradient) {
    /**
    ...
    * @param {Object} [gradient] - Gradient definition for the background or a background color, e.g. <code>{"-96": "rgb(30,87,153)", "-0.001": "rgb(41,137,216)", "0": "rgb(32,124,202)", "24": "rgb(125,185,232)"}</code>.
    *
    ...
    */
    ...
    if (typeof gradient === 'string' || gradient === false) {
      const bg = gradient ? gradient : 'transparent';
      ...
      if (element.tagName === 'CANVAS') {
        ...
        for (let i = 0; i < keys.length; i++) {
          let pos = this.valueToCoef(this.snap(keys[i]));
        ...
        for (let i = 0; i < keys.length; i++) {
          const ps = (100 * this.valueToCoef(this.snap(keys[i]))).toFixed(2);
          m_regular += sprintf(c_regular, gradient[keys[i] + ''], ps);
        }
        m_regular = m_regular.substr(0, m_regular.length - 2);
        element.style.background = sprintf(s_regular, d_w3c['s' + this.options.layout], m_regular);
      }
    }
