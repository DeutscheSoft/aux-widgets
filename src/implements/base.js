...
function remove_native_events(element) {
  ...
  for (type in s)
    if (is_native_event(type)) remove_event_listener(element, type, handler);

  for (type in d)
    if (is_native_event(type) && (!s || !s.hasOwnProperty(type)))
...
function add_native_events(element) {
  ...
  for (type in s)
    if (is_native_event(type)) add_event_listener(element, type, handler);
...
export const Base = define_class({
  ...
  initialized: function () {
    ...
    if (element !== this.__event_target) add_native_events.call(this, element);
  },
  /**
  ...
  set: function (key, value) {
     ...
     * @param {mixed} value - The value of the option.
     */
    if (this.has_event_listeners('set')) this.emit('set', key, value);
    /**
     * Is fired when an option is set.
     *
     ...
     */
    e = 'set_' + key;
    if (this.has_event_listeners(e)) this.emit(e, value, key);
  ...
  delegate_events: function (element) {
    ...
    if (old_target !== this.getEventTarget()) {
      throw new Error(
        'Cannot both overload getEventTarget() and call delegate_events()'
      );
    }
    /**
  ...
  on: function (event, func) {
    ...
    if (
      ...
      !this.has_event_listeners(event)
    )
