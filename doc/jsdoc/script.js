(function (w) {
  ...
  w.__init__ = function () {
    ...
    if (hash === 'run') {
      ...
      if (match && match.length) nav.scrollTop = parseInt(match[1]);
    }
  ...
  w.prepare_example = function () {
    window.addEventListener(
      'load',
      setup_example.bind(this, document.currentScript)
    );
  };
  ...
  function run_example() {
    ...
    if (window.example && window.example.root) window.example.root.destroy();
    window.example = {};
